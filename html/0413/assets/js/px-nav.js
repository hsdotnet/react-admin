+ function ($) {
    ...
    PxNav.prototype._setListeners = function () {
        ...
        }).on(Event.CLICK, Selector.ITEM_LINK, function (e) {
            e.preventDefault();
            var $this = $(this), li = $this.parent();

            if (!pxTab)
                pxTab = $('body > .px-content').data('px.tab');
            pxTab.add({ id: li.data('id'), icon: li.data('icon'), name: $this.find('.px-nav-label').text(), url: $this.attr('href'), closed: true });
        });
